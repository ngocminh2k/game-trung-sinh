...
export function GameScreen({ actionKind = null, actionNonce = 0, game, locale, chronicle, chronicleKinds, onAction, onLocaleChange, onRestart = () => {}, onExitToMenu, storyOpen = false, onStoryClose = () => {}, unlockedEndingIds, unlockedAchievementIds }: GameScreenProps) {
  ...
  useEffect(() => {
    const handleJournalShortcut = (event: globalThis.KeyboardEvent) => {
      ...
      if (event.key === 'Escape' && journalOpen) {
        ...
        // stopImmediatePropagation: after a re-render ProtoShell's window
        // listener can re-register *after* this one; both closing via
  ...
  function handleCellKeyDown(event: React.KeyboardEvent<HTMLButtonElement>, x: number, y: number) {
    ...
    const next = document.querySelector<HTMLButtonElement>(
      `.map-pin[data-pin-id="${String(x + dx)},${String(y + dy)}"]`,
    )
    next?.focus()
  }
